<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=Edge">
    <title>Category E-light</title>
     
<%@ include file="header.jsp" %>

    <div class="cover-page-list fl wfs bcf2">
        <div class="place fz12" style="padding-left:20px;">
    Your location:<a href=".">Homepage</a> <code>&gt;</code> <a href="/elight/category.jsp">Category</a></div>
                <div class="fl" style="width:300px;float:left;">
                    
                    <div class="left j-left wfs fz12">
                                                <h3 class="template">Items<span class="hide-left j-hide-left" style=""><i></i></span></h3>
                        <div class="icons j-icons wfs">
                            <a class="now" href="category.jsp?id=9">Local</a>
                            <a  href="category.jsp?id=6">Global</a>
                        </div>
                                                
                                                <div class="templates-items j-template-items wfs" style="display:block;">
                            
                                                         <dl class="conditions wfs">
                                <dt class="j-demo-l-dt wfs now"><b>Category</b></dt>
                                <dd class="j-demo-l-dd wfs" style="display:block;">
                                            <a href="/elight/category.jsp">All items</a>
                                            <a href="/elight/category.jsp">Sport</a>
                                            <a href="/elight/category.jsp">Cool</a>
                                            <a href="/elight/category.jsp">Pink</a>
                                            <a class="template-a" href="javascript:void(0);">Energy</a>
                                            <a href="/elight/category.jsp">Huge</a>
                                                                                                            </dd>
                            </dl>
                                               
                                                        
                            
                                                        <dl class="conditions wfs">
                                <dt class="j-demo-l-dt wfs now"><b>Price</b></dt>
                                <dd class="j-demo-l-dd wfs"  style="display:block;">
                           <a class="template-a" href="javascript:void(0);">All</a>
                 <li><a href="/elight/category.jsp">0&nbsp;-&nbsp;Less than 100</a></li>
                 <li><a href="/elight/category.jsp">20&nbsp;-&nbsp;100-200</a></li>
                 <li><a href="/elight/category.jsp">More than 200</a></li>
                                                                                                       </dd>
                            </dl>
                                                        
                        </div>
                                                
                        
                            <div class="also-like wfs mt20">
        <h4>Guess you want:</h4>
        <ul class="likes">
                    <li class="likes-li wfs">
                        <a target="_blank" href="/elight/ProductServelet?type=cool"><img src="style/images/2.gif"></a>
                        <strong class="fl wfs" style="text-align: center;"><a target="_blank" href="#">E-light cool</a></strong>
                        <i class="fl wfs"> 236 yuan  </i>
                    </li>
                    <li class="likes-li wfs">
                        <a target="_blank" href="/elight/ProductServelet?type=sport"><img src="style/images/3.gif"></a>
                        <strong class="fl wfs" style="text-align: center;"><a target="_blank" href="#">E-light sport</a></strong>
                        <i class="fl wfs"> 245 yuan  </i>
                    </li>
                    <li class="likes-li wfs">
                        <a target="_blank" href="/elight/ProductServelet?type=huge"><img src="style/images/4.gif"></a>
                        <strong class="fl wfs" style="text-align: center;"><a target="_blank" href="#">E-light huge</a></strong>
                        <i class="fl wfs"> 366 yuan  </i>
                    </li>
                    </ul>
    </div>
                        
                    </div>
                    
                </div>
                    
                    <div class="right j-right" style="margin-left: 300px;">
                    	<div  class="sort wfs">
                            <div style="float:left;">
                                <span style="margin-left:45px; float:left; padding-top:5px; font-size:12px;">Sort:</span>
                                
                                <span class="sort-icons j-sort-icons">
                                    <a class="now" rel="nofollow" href="category.jsp?category=9&display=grid&brand=0&price_min=0&price_max=0&filter_attr=0&page=1&sort=goods_id&order=ASC#goods_list">time</a>
                                    <a  rel="nofollow" href="category.jsp?category=9&display=grid&brand=0&price_min=0&price_max=0&filter_attr=0&page=1&sort=shop_price&order=ASC#goods_list">price</a>
                                    <a  rel="nofollow" href="category.jsp?category=9&display=grid&brand=0&price_min=0&price_max=0&filter_attr=0&page=1&sort=last_update&order=DESC#goods_list">click</a>
                                    <a  rel="nofollow" href="category.jsp?category=9&display=grid&brand=0&price_min=0&price_max=0&filter_attr=0&page=1&sort=sales_count&order=ASC#goods_list">order</a>
                                                                    </span>
                                
                            </div>
                            <div class="totals" style="padding-top:20px;margin-top:0px;text-align:right;">Totally <i>86</i> items</div>
                        </div>
                        
                            
   <form   name="compareForm" action="compare.php" method="post" onSubmit="return compareGoods(this);">
             <div class="boutique">
        <ul class="items wfs">
                             <li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=pink" style="position:relative;">
                                                                  <img src="style/images/3.gif" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=pink">Take you to Disney with our lamp</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light pink</a>
                                    </span>
                                    <p class="price-attente">Price:248$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li><li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=cool" style="position:relative;">
                                                                  <img src="style/images/2.gif" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=cool">Save energy,be cool with the environment</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light cool</a>
                                    </span>
                                    <p class="price-attente">Price:236$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                     $:                                                             </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li> <li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=sport" style="position:relative;">
                                                                  <img src="upload/item1.jpg" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=sport">Open your mind,Brighten your life</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light sport</a>
                                    </span>
                                    <p class="price-attente">Price:245$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li> <li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=huge" style="position:relative;">
                                                                  <img src="style/images/4.gif" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=huge">The big world,the huge-lamp</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light huge</a>
                                    </span>
                                    <p class="price-attente">Price:366$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li> <li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=pink" style="position:relative;">
                                                                  <img src="style/images/3.gif" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=pink">Take you to Disney with our lamp</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light pink</a>
                                    </span>
                                    <p class="price-attente">Price:200$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li><li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=cool" style="position:relative;">
                                                                  <img src="style/images/2.gif" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=cool">Save energy,be cool with the environment</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light cool</a>
                                    </span>
                                    <p class="price-attente">Price:236$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li><li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=sport" style="position:relative;">
                                                                  <img src="upload/item1.jpg" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=sport">Open your mind,Brighten your life</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light sport</a>
                                    </span>
                                    <p class="price-attente">Price:245$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li><li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=huge" style="position:relative;">
                                                                  <img src="style/images/4.gif" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=huge">The big world,the huge-lamp</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light huge</a>
                                    </span>
                                    <p class="price-attente">Price:366$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li>     
                        <li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=sport" style="position:relative;">
                                                                  <img src="upload/item1.jpg" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=sport">Open your mind,Brighten your life</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light sport</a>
                                    </span>
                                    <p class="price-attente">Price:245$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li> <li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=huge" style="position:relative;">
                                                                  <img src="style/images/4.gif" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=huge">The big world,the huge-lamp</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light huge</a>
                                    </span>
                                    <p class="price-attente">Price:366$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li> <li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=pink" style="position:relative;">
                                                                  <img src="style/images/3.gif" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=pink">Take you to Disney with our lamp</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light pink</a>
                                    </span>
                                    <p class="price-attente">Price:200$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li><li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=sport" style="position:relative;">
                                                                  <img src="upload/item1.jpg" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=sport">Open your mind,Brighten your life</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light sport</a>
                                    </span>
                                    <p class="price-attente">Price:245$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li> <li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=huge" style="position:relative;">
                                                                  <img src="style/images/4.gif" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=huge">The big world,the huge-lamp</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light huge</a>
                                    </span>
                                    <p class="price-attente">Price:366$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li> <li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=pink" style="position:relative;">
                                                                  <img src="style/images/3.gif" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=pink">Take you to Disney with our lamp</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light pink</a>
                                    </span>
                                    <p class="price-attente">Price:200$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li>
                        <li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=cool" style="position:relative;">
                                                                  <img src="style/images/2.gif" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=cool">Save energy,be cool with the environment</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light cool</a>
                                    </span>
                                    <p class="price-attente">Price:236$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li> <li class="items-li j-items-li">
                                                    <a target="_blank" href="/elight/ProductServelet?type=sport" style="position:relative;">
                                                                  <img src="upload/item1.jpg" title="商品TITLE">
                            </a>
                            <div class="wrap_div">
                                <div>
                                    <b class="title"><a target="_blank" href="/elight/ProductServelet?type=sport">Open your mind,Brighten your life</a></b>
                                    <span class="description">
                                      <a href="category.jsp" target="_blank" title="">E-light sport</a>
                                    </span>
                                    <p class="price-attente">Price:245$<br>city:Beijing</p>
                                </div>
                            </div>
                            <p class="price-attente">
                              <span class="price">
                                                                                                </span>
                              <span class="attente" title="浏览量">258</span>
                            </p>
                        </li>
                        
                                  </ul>
      </div>
        </form>
  
  
<script type="Text/Javascript" language="JavaScript">
<!--
function selectPage(sel)
{
  sel.form.submit();
}
//-->
</script>
                      
                        
                        
<form name="selectPageForm" class="pageForm" action="/elight/category.jsp" method="get">
 <div class="fl wfs">
        <ul class="pagination pages">
                                                                                                <li class="active"><a style="*margin-right: 5px;">1<span class="sr-only">(current)</span></a></li>
                                                                            <li><a href="/elight/category.jsp">2 <span class="sr-only">(current)</span></a></li>
                                                                            <li><a href="/elight/category.jsp">3 <span class="sr-only">(current)</span></a></li>
                                                                            <li><a href="/elight/category.jsp">4 <span class="sr-only">(current)</span></a></li>
                                                                            <li><a href="/elight/category.jsp">5 <span class="sr-only">(current)</span></a></li>
                                                                          <li><a href="/elight/category.jsp" style="*padding: 9px 12px;"><i class="page-bg page-right"></i> <span class="sr-only">(current)</span></a></li>
                    <li><a href="/elight/category.jsp">The end <span class="sr-only">(current)</span></a></li>    </ul>
</div>
</form>
<script type="Text/Javascript" language="JavaScript">
<!--
function selectPage(sel)
{
  sel.form.submit();
}
//-->
</script>
                        
                        <div style="clear:both;"></div>
                    </div>
                    
    </div>
    
<%@ include file="footernone.jsp" %>

<!--[if IE 6]> 
<script src="style/js/ie6-tips.js"></script>
<![endif]-->
<script>
        // 商品动画
        var move=function(wrap,cname){
           
            $("."+wrap).each(function(){
                var obj=$(this).find("."+cname);
                var p=obj.find("p");
                $(this).hover(function(){
                    p.stop().animate({height:20},200);
                    $(this).addClass("active");
                },function(){
                    p.stop().animate({height:0},200);
                    $(this).removeClass("active");
                });
            });
        }
        move("j-items-li","wrap_div");
        //模板条件切换
        $(".j-icons a").click(function(){
            $(this).addClass("now").siblings("a").removeClass("now");
            $(".j-template-items").eq($(this).index()).show().siblings(".j-template-items").hide();
        });
        // 排序切换
        $(".j-sort-icons a").click(function(){
            $(this).addClass("now").siblings("a").removeClass("now");
        });
        //左侧隐藏和展开
		$(".j-hide-left").click(function(){
			if($(".j-left").css('left')!="-300px")
			{
				$(".j-left").animate({left: '-300px'}, "slow");
				$(".j-right").animate({left:"-300px"},"slow");
				$(".j-right").css({width:"100%"});
				$(this).children().addClass('now');
			}else
			{
				$(".j-left").animate({left: '0'}, "slow");
				$(".j-right").animate({left:"0px"},"slow");
				$(".j-right").css({width:"auto"});
				$(this).children().removeClass('now');
			}
		})
        $(".j-demo-l-dt").click(
            function(){
				if($(this).hasClass('now')){
					$(this).removeClass("now");
					$(this).siblings("dd").hide();
				}else{
					$(this).addClass("now");
                	$(this).siblings("dd").show();
				}
                
                
                
            }
        );
        // $(".j-demo-l-dd a").click(function(){
        //      var span = "<span>"+$(this).html()+"</span>";
        //      $(this).parent("dd").siblings("dt").children("b").children("span").remove();
        //      $(this).parent("dd").siblings("dt").children("b").append(span);
        // });
    </script>
</body>
</html>
